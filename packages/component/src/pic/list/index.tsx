...
export function List(props: PicListProps) {
  ...
  const imgConfig = useMemo(() => {
    const data = { ...props };
    Reflect.deleteProperty(data, 'empty');
    Reflect.deleteProperty(data, 'value');
    Reflect.deleteProperty(data, 'fallback');
    return data;
  }, [props]);
      ...
      {value.length > 0
        ? value.map((item, index) => (
            <Image
              key={item || index}
              {...imgConfig}
              rootClassName={'ft-pic-item ' + (imgConfig.rootClassName || '')}
              src={item}
              fallback={fallback}
            />
          ))
